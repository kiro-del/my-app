...
// app/dashboard/create-rope-serials-v2/page.tsx
// Idea 2 prototype — tracks rope length per spool.
// Figma reference nodes: 3448-24978, 3448-24974 (context menu entry)
// Flow: Step 1 (Source rope detail) → Step 2 (Cut rope details)
...
interface Product {
  ...
  sku:             string;
  image?:          string;
  remainingLength: number;   // metres remaining on spool (mock data)
  trackingEnabled: boolean;  // whether length tracking is on
  /** Trace serial reference — when present renders as 3rd line (underlined), chip = "Item" */
  serialRef?:      string;
}

type SerialFormat = "customer" | "scannable" | "nfc";
type ProductMode  = "existing" | "new";
type SpoolUseMode = "current" | "new";

// ---------------------------------------------------------------------------
// Mock data
// ---------------------------------------------------------------------------
const RECENT_PRODUCTS: Product[] = [
  { id: "1", name: "Arbor Elite 12.7mm 60m One Sla...", brand: "Teufelberger", sku: "7361024",       remainingLength: 45,  trackingEnabled: true,  serialRef: "#AE241154B" },
  { id: "2", name: "ZENITH 9.5 mm Pink 60 m",           brand: "Beal",          sku: "BC095Z.60.P",  remainingLength: 32,  trackingEnabled: true,  serialRef: "#132241154A" },
  { id: "3", name: "ZENITH 9.5 mm Blue 200 m",          brand: "Beal",          sku: "BC095Z.200.B", remainingLength: 120, trackingEnabled: true,  serialRef: "#ZN200B0031" },
  { id: "4", name: "ZENITH 9.5 mm Blue 200 m",          brand: "Beal",          sku: "BC095Z.200.B", remainingLength: 0,   trackingEnabled: true,  serialRef: "#ZN200B0028" },
  { id: "5", name: "11MM Kernmantle Marlow Static R...", brand: "Marlow",        sku: "ZTO11/11",     remainingLength: 18,  trackingEnabled: false },
];

const GLOBAL_PRODUCTS: Product[] = [
  { id: "6", name: "Fixe Ranger Rope – 11mm – 200m Spool", brand: "Aspiring safety", sku: "P11FRSR",   remainingLength: 0,  trackingEnabled: false },
  { id: "7", name: "11mm Marlow Kernmantle static rope",    brand: "Marlow",          sku: "ZTO11/12",  remainingLength: 0,  trackingEnabled: false },
  { id: "8", name: "Teufelberger XSTATIC 11mm 100m",       brand: "Teufelberger",    sku: "7361025",   remainingLength: 0,  trackingEnabled: false },
  { id: "9", name: "Petzl PARALLEL 10.5mm 50m",            brand: "Petzl",           sku: "R075BA50",  remainingLength: 0,  trackingEnabled: false },
];

const CUT_ROPE_PRODUCTS: Array<{ id: string; name: string; brand: string; sku: string; lengthPerSerial: number }> = [
  ...
  return (
    <div
      style={{
        ...
        inset:          0,
        zIndex:         100,
        background:     "rgba(0, 0, 0, 0.4)",   // no DS overlay token — standard scrim value
        display:        "flex",
        alignItems:     "center",
...
// ---------------------------------------------------------------------------
// Add New Source Rope Modal
// ---------------------------------------------------------------------------
interface AddRopeModalProps {
  onClose: () => void;
  onAdd:   (rope: Product) => void;
}
function AddRopeModal({ onClose, onAdd }: AddRopeModalProps) {
  const [ropeSearch,        setRopeSearch]        = useState("");
  const [trackLength,       setTrackLength]        = useState(false);
  const [initialLength,     setInitialLength]      = useState("");
  const [initialLengthUnit, setInitialLengthUnit]  = useState("m");
  const [traceSerial,       setTraceSerial]        = useState("");

  function handleAdd() {
    const lengthM = trackLength && initialLength ? parseFloat(initialLength) : 0;
    const newRope: Product = {
      id:              `new-${Date.now()}`,
      name:            ropeSearch || "New source rope",
      brand:           "—",
      sku:             "—",
      remainingLength: lengthM,
      trackingEnabled: trackLength,
    };
    onAdd(newRope);
  }
  ...
  return (
    <ModalBackdrop onClose={onClose}>
      ...
      >
        <ModalHeader
          title="Add new source rope"
          bodyText="Add a new rope spool to the system. Enable length tracking to record how much rope is used over time."
          onClose={onClose}
        />
        ...
        >
          ...
          <ScanInput
            ...
            value={ropeSearch}
            onChange={(e) => setRopeSearch(e.target.value)}
          ...
          <ToggleInput
            label="Track length"
            description="Record the initial spool length to track remaining rope as it is cut."
            checked={trackLength}
            onChange={setTrackLength}
          />

          {/* Revealed: Initial length + Trace serial */}
          {trackLength && (
            <div style={{ display: "flex", flexDirection: "column", gap: tokens.spacing[4] }}>
              <CompositeInput
                ...
                unitValue={initialLengthUnit}
                onUnitChange={setInitialLengthUnit}
                unitPlaceholder="m"
              />
              <ScanInput
                label="Trace serial"
        ...
        <ModalFooter
          ...
          onCancel={onClose}
          onSubmit={handleAdd}
          submitDisabled={!ropeSearch.trim()}
        />
      </div>
    </ModalBackdrop>
  );
...
interface AddSpoolModalProps {
  onClose: () => void;
  onAdd:   (lengthM: number, mode: SpoolUseMode) => void;
}
function AddSpoolModal({ onClose, onAdd }: AddSpoolModalProps) {
  const [spoolLength,     setSpoolLength]      = useState("");
  const [spoolLengthUnit, setSpoolLengthUnit]  = useState("m");
  const [spoolSerial,     setSpoolSerial]      = useState("");
  const [useMode,         setUseMode]          = useState<SpoolUseMode>("current");

  function handleAdd() {
    const lengthM = parseFloat(spoolLength) || 0;
    onAdd(lengthM, useMode);
  }
  ...
  return (
    <ModalBackdrop onClose={onClose}>
      ...
      >
        <ModalHeader
          title="Add new spool"
          bodyText="Link an additional spool to continue tracking rope length for this batch."
          onClose={onClose}
        />
        ...
        >
          <CompositeInput
            ...
            placeholder="Enter length"
            value={spoolLength}
            onChange={setSpoolLength}
            unitOptions={LENGTH_UNITS}
            unitValue={spoolLengthUnit}
            onUnitChange={setSpoolLengthUnit}
            unitPlaceholder="m"
          />
          <ScanInput
            label="Trace serial"
            placeholder="Scan or enter serial"
            value={spoolSerial}
            onChange={(e) => setSpoolSerial(e.target.value)}
          />

          {/* Use of current rope radio */}
          <div style={{ display: "flex", flexDirection: "column", gap: tokens.spacing[2] }}>
            ...
            {(["current", "new"] as SpoolUseMode[]).map((mode) => (
              <div
                key={mode}
              ...
              >
                <RadioIndicator checked={useMode === mode} />
                <span
                  style={{
                    fontFamily: tokens.fontFamily.sans,
                    fontSize:   tokens.fontSize.body,
                ...
                >
                  {mode === "current" ? "Continue using current rope" : "Switch to new spool"}
                </span>
              </div>
            ))}
          </div>
        ...
        <ModalFooter
          ...
          onCancel={onClose}
          onSubmit={handleAdd}
          submitDisabled={!spoolLength.trim()}
        />
      </div>
    </ModalBackdrop>
  );
...
// ---------------------------------------------------------------------------
// Stop Tracking Length Confirmation Modal
// ---------------------------------------------------------------------------
interface StopTrackingModalProps {
  onClose:      () => void;
  onStopAndWipe: () => void;
}
function StopTrackingModal({ onClose, onStopAndWipe }: StopTrackingModalProps) {
  return (
    <ModalBackdrop onClose={onClose}>
      <div
        style={{
      ...
      >
        <ModalHeader
          title="Stop tracking length?"
          bodyText="This will wipe the remaining length data for this rope. You will no longer be able to track how much rope remains."
          onClose={onClose}
        />

        {/* Footer — custom: Keep tracking (secondary) + Stop tracking & wipe data (destructive) */}
        <div
          style={{
            display:        "flex",
            alignItems:     "center",
        ...
        >
          <Button variant="secondary" label="Keep tracking" onClick={onClose} />
          <button
            type="button"
            onClick={onStopAndWipe}
            style={{
              padding:      `${tokens.spacing[2.5]} ${tokens.spacing[4]}`,
              borderRadius: tokens.borderRadius.md,
              border:       `1px solid ${tokens.color.fg.red}`,
...
// Page
// ---------------------------------------------------------------------------
export default function CreateRopeSerialsV2Page() {
  ...
  const [trackingWiped,           setTrackingWiped]           = useState(false);

  // ── Modal visibility ────────────────────────────────────────────────────
  const [showStopTrackingModal, setShowStopTrackingModal] = useState(false);
  ...
  // Length tracking derived state
  const trackingEnabled  = selectedRope?.trackingEnabled && !trackingWiped;
  const remainingLength  = selectedRope?.remainingLength ?? 0;

  // Cut length per serial from selected product
  const cutLengthPerSerial: number = (() => {
    if (productMode === "existing" && selectedExistingProduct) {
      return CUT_ROPE_PRODUCTS.find((p) => p.id === selectedExistingProduct)?.lengthPerSerial ?? 0;
    }
    if (productMode === "new" && newLength) {
      return parseFloat(newLength) || 0;
    }
    return 0;
  })();

  const totalCutLength  = (parseInt(quantity, 10) || 0) * cutLengthPerSerial;
  const afterCutLength  = remainingLength - totalCutLength;
  const hasInsufficientLength =
    trackingEnabled && totalCutLength > 0 && afterCutLength < 0;
  const showLengthBanner =
    trackingEnabled && totalCutLength > 0 && !hasInsufficientLength;
  ...
  // ── Viewport flip: show dropdown above when near bottom ─────────────────
  useEffect(() => {
  ...
  const appBarProps = {
    breadcrumbs: [
      ...
      { label: "Create rope serials v2", href: "/dashboard/create-rope-serials-v2" },
    ],
    userInitials: "SW",
  };
  ...
  const renderStep1 = () => {
    const dropdownSections = searchResults
      ? [
          ...(searchResults.used.length   > 0 ? [{ label: "Item in inventory:",            items: searchResults.used   }] : []),
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
        ...
        >
          ...
          {/* Select source rope heading */}
          <div style={{ display: "flex", flexDirection: "column", gap: tokens.spacing[2] }}>
            <h3 style={h3Style}>Select source rope</h3>
            <p style={bodyStyle}>
              Search and select the original rope from Scannable database, or add a new spool.
            </p>
          </div>

          {selectedRope ? (
            /* ── Selected rope compact row ──────────────────────────────── */
            <div
              style={{
                display:       "flex",
                flexDirection: "column",
                ...
                action={
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setSearch("");
                      setSearchResults(null);
                      setTrackingWiped(false);
                    }}
                  />
                }
              />

              {/* Remaining length pill — only when tracking is enabled.
                  Wrapped in div so flex-col's align-items:stretch doesn't force Badge full-width. */}
              {selectedRope.trackingEnabled && (
                <div>
                  <Badge
                    color={selectedRope.remainingLength > 0 ? "green" : "red"}
                    label={
                      selectedRope.remainingLength > 0
                        ? `${selectedRope.remainingLength} m remaining`
                        : "Spool depleted"
                    }
                    withDot
                  />
            ...
            /* ── Search + grid ───────────────────────────────────────────── */
            <>
              {/* Search row — Figma node 3409:25241
                  Plain Input (no scan button) + primary Search button, half container width */}
              <div ref={searchContainerRef} style={{ position: "relative", width: "50%" }}>
              ...
              {/* Recent used products — card list
                  DS: ListViewItem (2-line or 3-line) inside a bordered card.
                  Figma: node 3414:26419 (Manufactures data file)
                  - serialRef present → 3-line variant + chip "Item" (green)
                  - serialRef absent  → 2-line variant + chip "Product" (gray)
                  - trackingEnabled → Badge for remaining length below ListViewItem
                  - Hover: "Select" button via ListViewItem action prop */}
              <div style={{ display: "flex", flexDirection: "column", gap: tokens.spacing[2], minWidth: 0 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: tokens.spacing[4] }}>
                  {RECENT_PRODUCTS.map((product) => {
                    ...
                    const hasSerial    = Boolean(product.serialRef);
                    const chipLabel    = hasSerial ? "Item" : "Product";
                    const chipColor    = hasSerial ? "green" : ("gray" as const);
                    ...
                    return (
                      <div
                        ...
                        onClick={() => setSelectedRope(product)}
                        style={{
                          display:       "flex",
                          flexDirection: "column",
                          gap:           tokens.spacing[3],     // 12px between ListViewItem and Badge
                          padding:       tokens.spacing[2],     // 8px — matches Figma p-spacing-2
                      ...
                      >
                        {/* ListViewItem — 2-line or 3-line depending on serialRef */}
                        <ListViewItem
                          imageUrl={product.image}
                          title={product.name}
                          subtitle={`${product.brand} · ${product.sku}`}
                          serialRef={product.serialRef}
                          chip={{ label: chipLabel, color: chipColor }}
                          showDivider={false}
                          action={
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); setSelectedRope(product); }}
                              style={{
                                ...
                                // Always in DOM to reserve width — hidden until hovered
                        ...
                        {/* Remaining length badge — below the list item, only when tracking.
                            Wrapped in div so flex-col's align-items:stretch doesn't force Badge full-width. */}
                        {product.trackingEnabled && (
                          <div>
                            <Badge
                              color={product.remainingLength > 0 ? "green" : "red"}
                              label={
                                product.remainingLength > 0
                                  ? `${product.remainingLength}m remaining`
                                  : "Depleted"
                              }
                            />
                          </div>
                ...
                tailingIcon={
                  <button
                    ...
                    style={{
                      background: "none",
                      border:     "none",
                      cursor:     sourceActive ? "pointer" : "not-allowed",
                      padding:    0,
                      display:    "flex",
                      alignItems: "center",
                    }}
  ...
  const renderStep2 = () => (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: tokens.spacing[2] }}>
          ...
          >
            <div style={{ display: "flex", gap: tokens.spacing[2], alignItems: "flex-start" }}>
              <div style={{ paddingTop: tokens.spacing[0.5], flexShrink: 0 }}>
        ...
        {/* Quantity + remaining length info */}
        <div style={{ display: "flex", flexDirection: "column", gap: tokens.spacing[3], width: "520px" }}>
          <Input
          ...
          {/* Length info banner — tracking on, sufficient length
              DS: Alert tone="info" type="compact" (nearest primitive — no "success" tone in DS Alert) */}
          {showLengthBanner && (
            <Alert
              tone="info"
              type="compact"
              ...
              withAction={false}
              title={`${afterCutLength.toFixed(1)} m remaining after this cut`}
            />
          )}

          {/* Insufficient length alert — DS Alert tone="disruptive" type="default" withAction */}
          {hasInsufficientLength && (
            <Alert
              tone="disruptive"
              type="default"
              withAction
              ...
              body={`This cut requires ${totalCutLength} m but only ${remainingLength} m remains on the spool.`}
              viewDetailLabel="Add new spool"
              dismissLabel="Stop tracking length"
              onViewDetail={() => setShowAddSpoolModal(true)}
              onDismiss={() => setShowStopTrackingModal(true)}
            />
          )}
        </div>
  ...
  return (
    <>
      ...
      {showAddRopeModal && (
        <AddRopeModal
          onClose={() => setShowAddRopeModal(false)}
          onAdd={(rope) => {
      ...
      {/* Stop tracking length confirmation modal */}
      {showStopTrackingModal && (
        <StopTrackingModal
          onClose={() => setShowStopTrackingModal(false)}
          onStopAndWipe={() => {
            setTrackingWiped(true);
      ...
      {showAddSpoolModal && (
        <AddSpoolModal
          onClose={() => setShowAddSpoolModal(false)}
          onAdd={(lengthM, mode) => {
            // Add the new spool's length to the selected rope's remaining
            if (selectedRope) {
              setSelectedRope({
                ...selectedRope,
                remainingLength: selectedRope.remainingLength + lengthM,
